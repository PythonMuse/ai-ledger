...
def make_hero():
    ...
    ax.text(0.50, 0.46, "TRUST  BOUNDARY", rotation=90, fontsize=15,
    ...
    ax.annotate("", xy=(0.462, header_center), xytext=(0.538, header_center),
                arrowprops=dict(arrowstyle="-|>", color=ALERT_RED, lw=2.6,
                                mutation_scale=19,
                                linestyle=(0, (4, 2))), zorder=3)
    ax.text(0.50, header_center, "X", fontsize=22, fontweight="bold",
            color=ALERT_RED, ha="center", va="center", zorder=5,
    ...
    arrow_v(ax, right_x + col_w / 2, rows_bottom - 0.018, wf_bottom + wf_h + 0.014,
            color=SEA_GREEN)
    ax.text(left_x + col_w / 2 + 0.026, (rows_bottom + wf_bottom + wf_h) / 2,
            "instructs", fontsize=15.5,
            fontweight="bold", color=OCEAN_TEAL, ha="left", va="center")
    ax.text(right_x + col_w / 2 + 0.026, (rows_bottom + wf_bottom + wf_h) / 2,
            "informs — data only", fontsize=15.5, fontweight="bold",
...
def make_invoice_walkthrough():
    ...
    card_top, card_bottom = 0.835, 0.225
    ...
    ax.text(left_x + card_w / 2, card_top - 0.050, "WHAT THE INVOICE CONTAINS",
    ...
    # they don't.
    row_top, row_pitch = card_top - 0.105, 0.064

    y = row_top
    for label, value in doc_lines:
        ax.text(left_x + 0.035, y, label, fontsize=16.5, color=OCEAN_TEAL,
                ha="left", va="center", zorder=4)
        ax.text(left_x + card_w - 0.035, y, value, fontsize=16.5,
                fontweight="bold", color=DEEP_NAVY, ha="right", va="center",
                zorder=4)
        y -= row_pitch

    inj_h = 0.205
    inj_bottom = card_bottom + 0.035
    ...
    ax.text(right_x + card_w / 2, card_top - 0.050,
            "WHAT THE APPROVED RULE COMPUTES", fontsize=18.5,
    ...
    for label, value in calc_lines:
        ax.text(right_x + 0.035, y, label, fontsize=16.5, color=WARM_GLOW,
                ha="left", va="center", zorder=4)
        ax.text(right_x + card_w - 0.035, y, value, fontsize=16.5,
                fontweight="bold", color=WHITE, ha="right", va="center",
                zorder=4)
        y -= row_pitch

    rounded_box(ax, (right_x + 0.03, inj_bottom + 0.015), card_w - 0.06, 0.135,
                GOLDEN_YELLOW, text_color=DEEP_NAVY,
                text="STATUS:  EXCEPTION", fontsize=20,
                sub="Routed for authorized human review",
                subsize=16, sub_color=OCEAN_TEAL,
                title_offset=0.17, sub_offset=0.26, zorder=3)

    rounded_box(ax, (0.04, 0.045), 0.92, 0.155, DEEP_NAVY,
                text_color=WHITE,
                text="The calculation lives outside the document, so the document cannot argue with it.",
                fontsize=18.5,
                sub="If approval genuinely exists, the workflow verifies it through the\nauthorization process — not through a sentence in the PDF.",
                subsize=15, sub_color=WARM_GLOW, linespacing=1.4,
                title_offset=0.28, sub_offset=0.20)
...
STAGES = [
    ...
    ("2.  AI RECOMMENDS", OCEAN_TEAL,
     ...
     "Control that matters:  reviewable inputs,\nand a reviewer who is not the model."),
    ("3.  AI ACTS", MIDNIGHT_TEAL,
     ...
     "Control that matters:  least privilege,\nand human approval on the action itself."),
]
...
def make_stages():
    ...
    for i, (name, color, doing, failure, control) in enumerate(STAGES):
        ...
        rounded_box(ax, (chip_x, y), chip_w, row_h, color,
                    ...
                    text=name, fontsize=19)
        rounded_box(ax, (body_x, y), body_w, row_h, LIGHT_GRAY,
                    text_color=DEEP_NAVY, text="", edge=OCEAN_TEAL, lw=1.1)
        ax.text(body_x + 0.028, y + row_h * 0.78, doing, fontsize=16.5,
                color=DEEP_NAVY, ha="left", va="center", zorder=4)
        ax.text(body_x + 0.028, y + row_h * 0.50, failure, fontsize=16.5,
                fontweight="bold", color=OCEAN_TEAL, ha="left", va="center",
                zorder=4)
        ax.text(body_x + 0.028, y + row_h * 0.185, control, fontsize=16.5,
                color=DEEP_NAVY, ha="left", va="center", zorder=4,
                linespacing=1.25)
    ...
    ax.text(0.011, (top + bottom_row_y) / 2, "CONSEQUENCE OF ERROR",
            rotation=90, fontsize=15.5, fontweight="bold", color=ALERT_ORANGE,
            ha="center", va="center")

    rounded_box(ax, (0.05, 0.0417), 0.90, 0.1713, DEEP_NAVY, text_color=WHITE,
                text="The more authority the AI receives, the stronger\nthe controls around its inputs must become.",
                fontsize=17, linespacing=1.3,
                sub="Read the arrow the other way too: an agent that can only read\nis an agent an injected instruction cannot spend money with.",
                subsize=15, sub_color=WARM_GLOW,
                title_offset=0.27, sub_offset=0.19)
...
def make_defense_in_depth():
    ...
    for i, (title, detail, kind, color) in enumerate(LAYERS):
        ...
        rounded_box(ax, (body_x, y), body_w, row_h, color, text_color=WHITE,
                    text=title, fontsize=17.5, sub=detail, subsize=15.5,
...
def make_social_square():
    ...
    add_header_bar(fig, "When the Invoice Gives Orders",
                   ...
                   height=header_h, title_size=23, subtitle_size=14.5,
                   brand=False)
